...
SamplerParams.AdjustmentLag=5000;
...
proposalScheme = RwmhProposal(eye(11,11),0);

%% Set up the sampler
MCMCsampler = AdaptiveSampler();

rwmh_adaption_samples=MCMCsampler.blockSample(SamplerParams,model,nuclearPumpData,proposalScheme,startParams);
save(strcat(getenv('P_HOME'), '/BayesianInference/Results/NuclearRwmhAdaption.mat'),'rwmh_adaption_samples','SamplerParams')