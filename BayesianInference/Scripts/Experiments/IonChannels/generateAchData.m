function filenames = generateAchData(model,concs,required_transitions,params)
    ...
    for set=1:length(concs)
        recording = generate_data(params,model,concs(set),required_transitions(set));
        [~,tmpname,~]=fileparts(tempname);
        filenames{set} = strcat(getenv('P_HOME'),'/BayesianInference/Data/AchGeneration/', num2str(required_transitions(set)),'_' ,tmpname,'_Synth_Ach_',num2str(concs(set)),'.scn');
