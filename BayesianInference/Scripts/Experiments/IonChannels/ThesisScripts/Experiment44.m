...
clear all;
experiment_description='Preconditioned RWMH at mode , Single Chain, 7-state concentration dependent, 3-concentrations, generated as per Hatton2003, Standard sampler';
...
generativeParams = true1(ten_param_keys);
desensParams = [generativeParams 5 1.4];

filenames = generateAchData(SevenState_10Param_QET(),data.concs(1:2),required_transitions(1:2),generativeParams);
filenames(3) = generateAchData(EightState_12Param_AT(),data.concs(3),required_transitions(3),desensParams);
...
[x,fval,exitflag] = fminsearch(@(params)-model.calcLogPosterior(abs(params),data),startParams,options);
...
save(strcat(getenv('P_HOME'), '/BayesianInference/Results/Thesis/Experiment44_' , num2str(t.Seed) , '.mat'))