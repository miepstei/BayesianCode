...
%% Model
model = ThreeStateQuickExactTensorIonModel();
startParams=[50;2000;2500;10000];

%% Data

data.tres = [ 0.000025 0.000025 0.000025];
data.concs = [1e-6 1e-5 1e-4 ];
data.tcrit =[1 1 1];
data.useChs=[0 0 1];
...
rng('shuffle')
t=rng;
samples=MCMCsampler.blockSample(SamplerParams,ThreeStateExactIonModel(),data,proposalScheme,startParams);
save(strcat(getenv('P_HOME'), '/BayesianInference/Results/ThreeState/mala_preconditioned' ,num2str(t.Seed()), '.mat'))